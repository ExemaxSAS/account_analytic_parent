...
class AccountAnalyticAccount(models.Model):
    ...
    analytic_line_ids = fields.Many2many('account.analytic.line', 'Cuentas analíticas propias e hijas',
                                         compute='_compute_analytic_line')

    @api.depends('child_ids.line_ids')
    def _compute_analytic_line(self):
        aline_ids = self.env['account.analytic.line'].search([('account_id', '=', self.id)])
        for child in self.child_ids:
            aline_ids += self.env['account.analytic.line'].search([('account_id','=', child.id)])
